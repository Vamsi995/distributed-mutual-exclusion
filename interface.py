...
class ClientInterface:
    ...
    def __init__(self, args, comm_factory: CommunicationFactory, banking_server: BankingServer, lamport_clock: LamportClock, pqueue: PriorityQueue, dictionary: BalanceTable, block_chain: BlockChain):
        ...
        self.root = tk.Tk()
        self.root.title(f"Client {lamport_clock.proc_id}")
        ...
        tk.Label(self.root, text="Key:").pack()
        self.receiver_entry = tk.Entry(self.root)
        self.receiver_entry.pack()

        tk.Label(self.root, text="Value:").pack()
        self.amount_entry = tk.Entry(self.root)
        self.amount_entry.pack()

        tk.Button(self.root, text="Insert <Key, Value>", command=self.perform_transaction).pack(pady=5)
    ...
    def perform_transaction(self):
        ...
        try:
            ...
            self.banking_server.transcation(self.lamport_clock, self.pqueue, self.dictionary, self.block_chain, receiver, amount, self.comm_factory, self)
